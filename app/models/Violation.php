...
class Violation {
    ...
    public function findActiveByGuard($guardId) {
        $stmt = $this->conn->prepare("SELECT v.*, u.full_name as student_name, u.profile_photo, s.student_id_number, s.course 
                                    ...
                                    JOIN users u ON v.student_user_id = u.id 
                                    JOIN students s ON u.id = s.user_id 
                                    WHERE v.guard_user_id = ? AND v.status != 'completed'
                                    ORDER BY v.created_at DESC");
        if (!$stmt) {
            error_log("Database error in findActiveByGuard: " . $this->conn->error);
            return null;
        }
        $stmt->bind_param("i", $guardId);
        $stmt->execute();
    ...
    public function findCompletedByGuard($guardId) {
        $stmt = $this->conn->prepare("SELECT v.*, u.full_name as student_name, u.profile_photo, s.student_id_number, s.course 
                                    ...
                                    ORDER BY v.created_at DESC");
        if (!$stmt) {
            error_log("Database error in findCompletedByGuard: " . $this->conn->error);
            return null;
        }
        $stmt->bind_param("i", $guardId);
        $stmt->execute();
    ...
    public function findByGuardName($guardName) {
        $stmt = $this->conn->prepare("SELECT v.*, u.full_name as student_name, u.profile_photo, s.student_id_number, s.course 
                                    FROM violations v 
                                    JOIN users u ON v.student_user_id = u.id 
                                    JOIN students s ON u.id = s.user_id 
                                    WHERE v.recorded_by_guard_name LIKE ?
                                    ORDER BY v.created_at DESC");
        $likeName = "%$guardName%";
